...
function formatSet(meta, questions) {
    ...
    lines.push(
        '// Re-run the extractor to refresh (existing in-app edits are saved'
    );
    lines.push('// to localStorage and survive regeneration).');
    lines.push(
...
function main() {
    ...
    if (!answers) {
        ...
        log(
            'or fix each question in-app with the ✏️ Editar button (persists per browser).'
        );
    }
